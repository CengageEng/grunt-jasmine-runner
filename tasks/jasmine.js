...
function task(grunt) {
    ...
    grunt.registerTask('jasmine-server', 'Run jasmine specs interactively in the default browser.', function () {
        var done = this.async();
        task.interactiveRunner(grunt.config('jasmine'));
    });

    grunt.registerTask('jasmine-server-silent', 'Set up jasmine runner but do not open a browser.', function () {
        var done = this.async();
        task.nonInteractive(grunt.config('jasmine'));
    });
...
task.nonInteractive = function(options,cb){
    options = grunt.util._.extend({},defaultOptions,options);

    jasmine.buildSpecrunner(baseDir, options, []);
    grunt.log.writeln('SpecRunner constructed');
